Following the Introduction's exploration of AI's emerging role in software architecture, this Literature Review aims to deepen our understanding. It sets out to map the current landscape of AI advancements and software pattern methodologies, framing the context for our research. The section provides a thorough overview of two key areas: the evolution of AI, particularly \ac{LLM} and Prompt Engineering, and the selection methods of software patterns. This review will underpin our investigation into AI-assisted tools for design decision-making in software projects.

\textbf{Evolution of AI and Large Language Models:} Examines how AI has evolved, focusing on large language models and their impact on software development.~\textbf{Prompt Engineering in AI:} Discusses prompt engineering principles and their applications in software development.~\textbf{Tools for Developers:} Reviews current AI tools for software development.~\textbf{Software Patterns and Design Pattern Selection Methods:} Traces the history of software patterns and analyzes methods for selecting design patterns, setting the stage for our AI-focused exploration in subsequent sections.

\section{An Overview of the Evolution of AI}\label{sec:AI-evolution}
AI is a broad field encompassing the study and creation of systems that can perform tasks that usually require human intelligence, such as reasoning, learning, perception, and decision-making. The term was coined by John McCarthy in 1956, who defined it as ``the science and engineering of making intelligent machines.''~\cite{Mccarthy2006}. Since then, AI has evolved into various subfields and paradigms, such as symbolic AI, connectionism, evolutionary computation, and machine learning. 
...
Natural Language Processing is a subfield of AI that deals with the interaction between computers and human languages. Natural Language Processing aims to enable systems to understand, analyze, generate, and manipulate natural language texts and speech. NLP has many applications in various domains~\cite{Devlin2018}, such as information retrieval, sentiment analysis, summarization, dialogue systems, chatbots, and natural language understanding. Natural Language Processing also involves studying the linguistic and cognitive aspects of human language processing and communication.
...
In software design, prompt engineering becomes particularly significant. Crafting a well-structured prompt that accurately encapsulates the design problem can guide the AI tool in suggesting an appropriate design pattern. This saves developers time and effort and helps ensure that the software design is robust and efficient.

It's important to note that there is no one-size-fits-all solution for engineering solutions quickly. Different situations may call for different requests to achieve the best possible results. There are five key principles to remember when constructing effective prompts. These principles, known as the ``Five Principles of Prompting''~\cite{Phoenix2024} are as follows:
...
% Start Figure ----------------------------
\begin{figure}[ht]
\caption{Example of a Few-Shot Learning Prompt Question}
\centering
...
% Start Figure ----------------------------
\begin{figure}[ht]
\caption{Responses to a Few-Shot Learning Prompt}
\centering
...
An analysis of programmer interactions with Copilot showed that the effectiveness of auto-completion depended on whether developers were speeding up familiar tasks ``acceleration mode'' or exploring unfamiliar problem solutions ``exploration mode''. Developers often iteratively type comments, review Copilot's responses and modify their comments to explore other potential solutions. These comments are usually deleted after the relevant code is generated, reflecting their temporary function in this iterative refinement process.

However, these models' imperfection can lead to challenges. They often struggle to distinguish correct outputs from seemingly coherent and plausible ones, leading to potential bugs or security vulnerabilities. This has gained widespread attention since the release of ChatGPT.
...
Patterns are not an exclusive aspect of Software Engineering. Psychologists identify behavior patterns upon which they can take one action or another. Economists look for patterns in changes in stock prices that support their buying and selling policies. 
...
\textit{``Each pattern describes a problem which occurs over and over again in our environment, and then describes the core of the solution to that problem, in such a way that you can use this solution a million times over, without ever doing it the same way twice."}~\cite[10]{Alexander1977}. 
...
\ac{DP} have been organized into three categories, each defining different problems. They are categorized into creational, structural, and behavioral patterns, described in Table~\ref{board:pattern-category} above:
...
\MyParaBold{UML-Based Approach}\label{subsec:uml-approach}
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Schematic of the UML-Based Design Pattern Suggestion Approach.}
\centering
\label{fig-uml-based-approach}
\includegraphics[width=0.9\textwidth]{figures/uml-based-selecion-pattern-approach.png}
\objectsource{Elaborated by the author, inspired by \textcite{Naghdipour2023Semi}.}
\end{figure}
% Fim Figura ---------------------------
...
\MyParaBold{Question-Answer-Based Approach}\label{subsec:question-answer-approach}
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Flowchart of the Question-Answer Based Design Pattern Selection Approach.}
\centering
...
\MyParaBold{CBR-Based Approach}\label{subsec:cbr-approach}

The Case-Based Reasoning Approach uses old experiences to understand and solve new problems. Each of the experiences stored in memory is called a case. Each case in case-based reasoning represents a situation that can be taken into different formats based on experience. Typically, each case consists of three parts: the problem, the solution, and the explanation that states how the solution solves the problem~\cite{Kolodner1992}. The process of pattern suggestion in this approach is shown in Figure~\ref{fig-cbr-approach}. A case base can have different formats, such as text, diagram, and ontology.

% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Process Diagram of the Case-Based Reasoning Approach in Design Pattern Selection.}
\centering
...
As a result, the Case-Based Reasoning system is enriched and improved by adding new cases to the case base~\cite{Avramenko2004}. ~\textcite{Muangon2013} has used this approach to suggest suitable design patterns.~\textcite{Gomes2002} presented a REBUILDER tool that provides new functionalities based on Case-Based Reasoning. The proposed method saves cases in a case base and is indexed using WordNet. Its knowledge base is constructed using examples of problem-specific cases to which design patterns are applied. 
...
\MyParaBold{Anti-Patterns-Based Approach}\label{subsec:anti-pattern-approach}
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Flowchart of the Anti-Patterns Approach for Design Pattern Selection.}
\centering
...
\MyParaBold{Text-Classification-Based Approach}\label{subsec:text-classification-approach}
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Schematic of the Text Classification Approach for Design Pattern Selection.}
\centering
...
\MyParaBold{Information-Retrieval-Based Approach}\label{subsec:retrieval-approach}
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Flowchart of the Information Retrieval-Based Approach for Design Pattern Selection.}
\centering
...
\MyParaBold{Ontology-Based Approach}\label{subsec:ontology-approach}

Ontology is very popular in computer science. It enables domain knowledge to be modeled abstractly and queries to be evaluated against a knowledge base. There are various tools and methods for creating an ontology domain. The Web Ontology Language (OWL) is a standard language for ontology representation whose primary syntax is XML/RDF (resource description framework) and is used to model knowledge of a particular domain. 
...
% Inicio Figura ---------------------------
\begin{figure}[ht]
\caption{Diagram of the Ontological Approach in Pattern Analysis.}
\centering
...
However, the designer can only use the concepts defined in the ontology.~\textcite{Bou2018} preprocessed the constructed ontology and the design problem to create feature vectors. They suggested design patterns based on the similarity between design patterns and design problem vectors.~\textcite{Blomqvist2008} created an ontology for the design problem and tried to find the similarity between the constructed ontology from patterns and problems.
 ...
 \MyParaBold{Others Approaches}\label{subsec:others-approach}
...
\MyParaBold{Combination of Approaches}\label{subsec:combination-approach}
...
\MyParaBold{Trade-offs between the Design Patterns Selection Approaches} \label{subsec:trade-offs-approaches}

\textcite{Kim2007} presented a UML-based method that utilizes Role-Based Modeling Language (RBML) to create the meta-model for each pattern and problem. They used collaboration and class diagrams to specify different aspects of patterns, resulting in different properties, such as structural and behavioral. Finally, a pattern is suggested if the design pattern's specifications can meet the problem's diagram. However, the method's scalability could be improved due to overlapping meta-models.
...
Semi-automatic methods that require interaction with developers or designers are also a limitation. The ideal approach would be fully automated, allowing large-scale applications without manual intervention. To achieve this, techniques such as text mining, clustering, \ac{ML}, and AI can be employed. These automated techniques can handle large amounts of data and accelerate implementation. The goal is to improve efficiency and accuracy in pattern suggestion by exploring different methods and algorithms related to these techniques~\cite {Naghdipour2023}.

Despite the advances in design pattern selection, there are still concerns regarding the focus on specific patterns and issues related to cost, time, comprehensiveness, and method evaluation. These areas require further improvement and exploration to enhance the effectiveness and practicality of design pattern selection approaches.

\subsection{Section Summary}

Section 2 of this dissertation provides a literature review divided into two main sections: the evolution of artificial intelligence (AI) and methodologies for software pattern selection.

\MyParaBold{Evolution of AI}

This subsection begins with an overview of the historical development of AI, emphasizing the significant milestones and progression from early symbolic AI to modern machine learning (ML) and deep learning (DL) techniques. It details the emergence and capabilities of large language models (LLMs) and their transformative effects on natural language processing and software development tasks.
    
Special attention is given to prompt engineering, described as the art and science of crafting prompts to guide AI models such as GPT-4. Effective, prompt engineering is shown to be crucial in improving AI performance by narrowing the scope of responses and focusing on relevant aspects of the problem at hand, enlightening you about this important aspect of AI development.


\MyParaBold{Software Patterns}

The review delves deeper into the concept of software patterns, beginning with their origins in Christopher Alexander's work, which introduced the idea of patterns in architecture. This section explains how these ideas were adapted to software engineering, mainly through the Gang of Four (GoF) influential work, which categorized design patterns into creational, structural, and behavioral types.

It further explores various methods for selecting appropriate design patterns, such as UML-based approaches, question-and-answer-based methods, case-based reasoning, anti-patterns, and information retrieval-based approaches. The advantages and limitations of each method are discussed, equipping you with a comprehensive understanding of the tools available to software architects for pattern selection.

Overall, Section 2 lays the foundation for the dissertation's exploration of how generative AI can aid architectural decision-making, providing a thorough understanding of AI's capabilities and the established methodologies for using design patterns in software development.
